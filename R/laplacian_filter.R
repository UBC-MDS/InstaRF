...
laplacian_filter <- function(input_img, output_img) {
  ...
  for (i in c(1:img_ydim)) {
    for (j in c(1:img_xdim)) {
      output[i,j,1] <- input[i,j,1]*4 - 
        input[max(1,       i-1), j                , 1] - 
        input[min(img_ydim,i+1), j                , 1] - 
        input[i                , max(1,       j-1), 1] -
        input[i                , min(img_xdim,j+1), 1]
    
      output[i,j,2] <- input[i,j,2]*4 - 
        input[max(1,       i-1), j                , 2] - 
        input[min(img_ydim,i+1), j                , 2] - 
        input[i                , max(1,       j-1), 2] -
        input[i                , min(img_xdim,j+1), 2]
      
      output[i,j,3] <- input[i,j,3]*4 - 
        input[max(1,       i-1), j                , 3] - 
        input[min(img_ydim,i+1), j                , 3] - 
        input[i                , max(1,       j-1), 3] -
        input[i                , min(img_xdim,j+1), 3]
    }
  }
  
  #save the output
  writePNG(output, target=output_img)
}
